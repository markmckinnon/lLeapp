...
def get_wtmp(files_found, report_folder, seeker, wrap_text):
    ...
    for file_found in files_found:
        ...
        with open(file_found, 'rb') as f:
            ...
            for entry in utmp.read(buf):
                ...
                if entry.user == 'LOGIN' or entry.user == '' or entry.user == 'runlevel':
                    continue
        ...
        if usageentries > 0:
            report = ArtifactHtmlReport(f'wtmp History')
            #check for existing and get next name for report file, so report from another file does not get overwritten
            report_path = os.path.join(report_folder, f'wtmp.temphtml')
            ...
            tsvname = f'wtmp History'
            tsv(report_folder, data_headers, data_list, tsvname)
            
            tlactivity = f'wtmp History'
