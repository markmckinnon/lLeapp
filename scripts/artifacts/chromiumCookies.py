...
def get_chromeCookies(files_found, report_folder, seeker, wrap_text):
    ...
    for file_found in files_found:
        ...
        if usageentries > 0:
            report = ArtifactHtmlReport(f'{browser_name} Cookies - {user_name}')
            #check for existing and get next name for report file, so report from another file does not get overwritten
            report_path = os.path.join(report_folder, f'{browser_name} Cookies - {user_name}.temphtml')
            report_path = get_next_unused_name(report_path)[:-9] # remove .temphtml
            report.start_artifact_report(report_folder, os.path.basename(report_path))
            report.add_script()
            data_headers = ('Last Access Date','Host','Name','Value','Created Date','Expiration Date','Path', 'username', 'sourcefile')
            data_list = []
            for row in all_rows:
                if wrap_text:
                    data_list.append((row[0],row[1],(textwrap.fill(row[2], width=50)),textwrap.fill(row[3], width=50),row[4],row[5],row[6], user_name, source_file))
                else:
