...
def get_chromeDownloads(files_found, report_folder, seeker, wrap_text):
    ...
    for file_found in files_found:
        ...
        if usageentries > 0:
            report = ArtifactHtmlReport(f'{browser_name} Downloads - {user_name}')
            #check for existing and get next name for report file, so report from another file does not get overwritten
            report_path = os.path.join(report_folder, f'{browser_name} Downloads - {user_name}.temphtml')
            report_path = get_next_unused_name(report_path)[:-9] # remove .temphtml
            report.start_artifact_report(report_folder, os.path.basename(report_path))
            report.add_script()
            data_headers = ('Start Time','End Time','Last Access Time','URL','Target Path','State','Opened?','Received Bytes','Total Bytes', 'username', 'sourcefile')
            data_list = []
            for row in all_rows:
                data_list.append((row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8], user_name, source_file))
